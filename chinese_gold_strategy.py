...
class commodity_strategy:
    ...
    def __init__(
        ...
        if not latest_data.empty:
            self.ticker_close_price = spot_quotations_sge_df[spot_quotations_sge_df['时间'] == spot_quotations_sge_df['时间'].max()]['现价']
        else:
    ...
    def break_point_solution(self):
        ...
        df = df[df['date'].between(pd.to_datetime(start), pd.to_datetime(end))]
    ...
    def plot_chart(self, interactive_plot):
        ...
        * MACD
        ...
        if self.strategy == 'daily':
            ...
            ax.set_title(self.commodity_code.upper())
            ...
            ax1.set_title('Daily commodity price for ' + self.commodity_code.upper())
            ax1.legend()
    ...
    def infer_metric(self, realtime=True, imputed_value=None, print_result=True):
        ...
        today_date = datetime.today()
        tomorrow_date = datetime.today() + relativedelta(days=1)
